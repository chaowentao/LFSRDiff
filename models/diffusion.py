...
class GaussianDiffusion(nn.Module):
    def __init__(self, denoise_fn, enc_net, timesteps=1000, loss_type="l1"):
        super().__init__()
        ...
        # Append 1.0 to the beginning of alphas_cumprod and take the cumulative product.
    ...
    def forward(self, img_hr, img_lr, img_lr_up, t=None, *args, **kwargs):
        ...
        if hparams["use_enc"]:
            if hparams["fix_enc"]:
                self.enc.eval()
                with torch.no_grad():
                    enc_out, cond = self.enc(img_lr, True)
                    if "use_enc_ups" in hparams.keys() and hparams["use_enc_ups"]:
                        img_lr_up = enc_out
            else:
                enc_out, cond = self.enc(img_lr, True)
        else:
        ...
        p_losses, x_tp1, noise_pred, x_t, x_t_gt, x_0 = self.p_losses(
            x, t, cond, img_lr_up, *args, **kwargs
        )
        ret = {"q": p_losses}
        if not hparams["fix_enc"]:
            if hparams["aux_l1_loss"]:
                ret["aux_l1"] = F.l1_loss(enc_out, img_hr)
            if hparams["aux_ssim_loss"]:
                ret["aux_ssim"] = 1 - self.ssim_loss(enc_out, img_hr)
            if hparams["aux_percep_loss"]:
                ret["aux_percep"] = self.percep_loss_fn[0](img_hr, enc_out)
    ...
    def p_losses(self, x_start, t, cond, img_lr_up, noise=None):
        ...
        x_t_pred, x0_pred = self.p_sample(
            x_tp1_gt, t, cond, img_lr_up, noise_pred=noise_pred
        )
